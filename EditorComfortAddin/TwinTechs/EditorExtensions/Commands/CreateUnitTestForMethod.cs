...
namespace TwinTechs.EditorExtensions.Commands
{
	public class CreateUnitTestForMethod : CommandHandler
	{
		...
		void CreateTestMethod(string className, string methodName)
		{
			...
			//FIXME work out how to get text editor data from editor - this might not even be possible anymore!
			var mode = new InsertionCursorEditMode(editor,
			...
			mode.Exited += delegate (object s, InsertionCursorEventArgs iCArgs)
			{
				if (iCArgs.Success)
				{
					...
					//FIXME - how to insert text?
					var offset = document.Editor.LocationToOffset(iCArgs.InsertionPoint.Location.Line, iCArgs.InsertionPoint.Location.Column);
					document.Editor.InsertText(offset, methodText);
					editor.SetCaretLocation(iCArgs.InsertionPoint.Location.Line, iCArgs.InsertionPoint.Location.Column);
				}
			};
		}
